...
import { getCreche, logout, restoreAuthSession } from "@/lib/api";
...
export default function TabletLayout({
  ...
  useEffect(() => {
    restoreAuthSession().then((hasSession) => {
      const creche = getCreche();
      if (!hasSession || !creche) router.push("/");
      else setCrecheName(creche.nom);
    });
  }, [router]);
