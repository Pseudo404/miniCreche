...
export default function AdminEmployeeDetail() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [timeRes, sigRes] = await Promise.all([
          apiFetch(`/admin/employees/${id}/time/?month=${month}&year=${year}`),
          apiFetch(`/admin/employees/${id}/emargements/?month=${month}&year=${year}`),
        ]);
        setTimeData(timeRes);
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      ...
      try {
        const data: ScheduleEntry[] = await apiFetch(`/admin/employees/${id}/schedule/`);
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      await apiFetch(`/admin/employees/${id}/schedule/`, {
        method: "POST",
      ...
      const timeRes = await apiFetch(
        `/admin/employees/${id}/time/?month=${month}&year=${year}`
      );
      setTimeData(timeRes);
    } catch (err) {
      console.error(err);
