...
export async function saveEmargementLocally(record: Omit<EmargementRecord, 'id' | 'creche_name' | 'synced'>) {
    ...
    const eventsToday = emargements.filter((event) =>
        ...
        event.creche_name === (creche?.nom || 'Inconnue') &&
        new Date(event.timestamp).toLocaleDateString('en-CA') === today
    );

    if (eventsToday.some((event) => event.type_event === record.type_event)) {
        const label = record.type_event === "ARRIVEE" ? "arrivée" : "départ";
        throw new Error(`La signature d’${label} a déjà été enregistrée aujourd’hui.`);
    }
